...
def _font(bold: bool = False, sz: int = 10, color: str = "FF000000", white: bool = False) -> Font:
    return Font(
        name="Arial",
        bold=bold,
        size=sz,
...
def load_staffing_schedule(path: str) -> pd.DataFrame:
    ...
    """
    raw = pd.read_excel(path, sheet_name=0, header=25)
    raw.columns = (
        ["Строка", "Подразд1", "Подразд2", "Период", "Должность",
         "ЧТС", "Оклад", "МесТС", "ШтатЕд", "Итого"]
        + [f"x{i}" for i in range(raw.shape[1] - 10)]
    )

    skip_job = {
        "", "nan",
        "Руководители, специалисты, служащие", "Рабочие", "РСС",
        "постоянно", "отопительный период", "межотопительный период",
        "Должность/профессия",
    }
    ...
    for _, row in raw.iterrows():
        p1 = _clean(row["Подразд1"])
        p2 = _clean(row["Подразд2"])
        job = _clean(row["Должность"])
        units = row["ШтатЕд"]
        period = _clean(row["Период"])

        # Строка-заголовок отдела: p1 — название (не "1", не цифра), p2 пустой
        if p1 and p1 not in ("1", "nan") and not p1.isdigit() and p2 in ("nan", ""):
            dept = p1
            subdept = ""
            continue

        # Строка-заголовок подотдела: p2 — название (не "1", не цифра)
        if p2 and p2 not in ("1", "nan") and not p2.isdigit():
            subdept = p2
            continue
        ...
        pay = _safe_pay(row["Оклад"], row["ЧТС"])
        effective = subdept if subdept else dept

        rows.append({
            "Отдел":        dept,
            "Подотдел":     subdept,
            "Подразделение": effective,
            "Должность_ШР": job,
            "Оклад":        pay,
            "ШтатЕд":       float(units),
            "Период":       period,
    ...
    print(f"[ШР] Загружено позиций: {len(df)},  штатных единиц: {df['ШтатЕд'].sum():.2f}")
    return df
...
def load_payroll(path: str) -> pd.DataFrame:
    ...
    raw.columns = (
        ["№", "Категория", "Подразд1", "Подразд2", "Подразд3",
         "Профессия", "ТабН", "ФИО"]
...
def match(sr_df: pd.DataFrame, emp: pd.DataFrame):
    ...
    print(f"[Сопоставление] Занято: {occ},  Вакансий: {vac},  Без позиции в ШР: {len(unmatched)}")
...
def _write_main_sheet(ws, out_df: pd.DataFrame):
    ...
    headers = [
        "Отдел / Подразделение", "Должность / профессия",
        "Штат.\nед.", "Оклад,\nруб.", "Период действия",
        "Таб. №", "ФИО", "Статус",
    ]
    ...
    for _, r in out_df.iterrows():
        ...
        specs = [
            ...
            (5, period,  CLR["yellow"] if period != "постоянно" else rf,
                                        _font(sz=9),                                      _center()),
            (6, tabn,    rf,            _font(sz=10),                                     _center()),
            (7, fio,     rf,            _font(bold=not occ, sz=10,
                                              color="FF376923" if occ else "FFBF1F1F"),   _left()),
            (8, status,  rf,            _font(bold=True, sz=10,
                                              color="FF376923" if occ else "FFBF1F1F"),   _center()),
        ]
...
def _write_summary_sheet(ws, out_df: pd.DataFrame, sr_df: pd.DataFrame):
    ...
    for _, row in summary.iterrows():
        ...
        for ci, val in enumerate([dept, штат, заня, вак, f"{pct} %"], 1):
            c = ws.cell(row=rn, column=ci, value=val)
            _style(c, CLR["white"] if ci == 1 else rf, _font(sz=10),
    ...
    for ci, val in enumerate(["ИТОГО", round(total_u, 2), total_z, total_v, f"{total_p} %"], 1):
        c = ws.cell(row=rn, column=ci, value=val)
        _style(c, CLR["dept"], _font(bold=True, sz=11, white=True),
               _left() if ci == 1 else _center())
...
def _write_unmatched_sheet(ws, unmatched: pd.DataFrame):
    ...
    _style(c, CLR["yellow"],
           Font(name="Arial", italic=True, size=10, color="FF7F6000"), _left())
    ...
    for _, e in unmatched.sort_values(["Подразд3", "Профессия"]).iterrows():
        ...
        for ci, val in enumerate(
            ...
            c = ws.cell(row=rn, column=ci, value=val)
            _style(c, CLR["vac"], _font(sz=10),
                   _left() if ci in (1, 2, 4) else _center())
        rn += 1
